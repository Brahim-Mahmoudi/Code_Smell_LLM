...
def rule_R20(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if (((isSubscript(sub) and isChainedIndexingBase(sub)) and isDataFrameVariable(get_base_name(sub.value), sub))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Avoid chained indexing in DataFrames at line {lineno}; use df.loc[]", node)
