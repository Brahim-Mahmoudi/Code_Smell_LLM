...
def rule_R16(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if (((isApiMethod(sub) and (not hasInplaceTrue(sub))) and (not isResultUsed(sub)))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("API call might be missing reassignment or inplace=True at line {lineno}", node)
