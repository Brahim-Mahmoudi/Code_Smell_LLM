...
def rule_R3(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((((isForLoop(sub) or isFunctionDef(sub))) and hasConstantAndConcatIntersection(sub))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("TensorArray Not Used at line {lineno}", node)
