...
def rule_R13(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if (isinstance(sub, ast.Assign) and ((isDataFrameColumnAssignment(sub) and ((isAssignedLiteral(sub, 0) or isAssignedLiteral(sub, "")))))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Detected a new column initialized with a filler (0 or ''). Use np.nan or an empty Series instead at line {lineno}", node)
