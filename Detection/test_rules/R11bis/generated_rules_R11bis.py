...
def rule_R11bis(ast_node):
    import ast
    add_parent_info(ast_node)
    #set_deterministic_flag(ast_node)
    # "Data Leakage (no pipeline in presence of model.fit)"
    variable_ops = gather_scale_sensitive_ops(ast_node)
    scaled_vars = gather_scaled_vars(ast_node)
    model_fit_present = isModelFitPresent(ast_node)
    pipeline_used = pipelineUsedGlobally(ast_node)
    problems = {}
    for sub in ast.walk(ast_node):
        if (((model_fit_present and (not pipeline_used)) and isFitCall(sub))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Potential data leakage: model.fit() used without a pipeline at line {lineno}", node)
