...
def rule_R29(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((isUnstructuredLLMCallInPipeline(sub) and isTextGeneratingCall(sub))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("LLM pipeline call without structured output at line {lineno}", node)
