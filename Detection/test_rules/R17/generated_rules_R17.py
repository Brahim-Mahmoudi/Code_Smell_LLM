...
def rule_R17(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((isForLoop(sub) and (((((usesIterrows(sub.iter) and isDataFrameVariable(get_base_name(sub.iter), sub.iter))) or ((usesItertuples(sub.iter) and isDataFrameVariable(get_base_name(sub.iter), sub.iter)))) or usesPythonLoopOnTensorFlow(sub))))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Unnecessary iteration detected; consider using vectorized operations (e.g., DataFrame.add, tf.reduce_sum) for efficiency at line {lineno}", node)
