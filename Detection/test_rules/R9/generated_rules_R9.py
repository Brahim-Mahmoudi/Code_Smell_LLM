...
def rule_R9(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((isLossBackward(sub) and (not hasPrecedingZeroGrad(sub)))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("optimizer.zero_grad() not called before loss_fn.backward() at line {lineno}", node)
