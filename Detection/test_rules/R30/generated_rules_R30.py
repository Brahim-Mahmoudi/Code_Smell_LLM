...
def rule_R30(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((isReasoningModelCall(sub) and hasNoReasoningEffort(sub))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Reasoning model call without explicit reasoning effort parameter at line {lineno}", node)
