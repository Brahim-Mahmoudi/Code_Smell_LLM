...
def rule_R28(ast_node):
    ...
    # "LLM Calls Without Bounded Metrics"
    variable_ops = gather_scale_sensitive_ops(ast_node)
    scaled_vars = gather_scaled_vars(ast_node)
    problems = {}
    for sub in ast.walk(ast_node):
        if (((isLLMCall(sub) and hasNoBoundedMetrics(sub)) and isNotSDKClient(sub))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("LLM call without bounded metrics (tokens/timeout/response size) at line {lineno}", node)
