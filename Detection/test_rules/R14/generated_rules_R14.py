...
def rule_R14(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((isValuesAccess(sub) and isDataFrameVariable(get_base_name(sub), sub))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Avoid using .values on DataFrames. Use .to_numpy() instead at line {lineno}", node)
