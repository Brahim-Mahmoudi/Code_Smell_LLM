...
def rule_R5(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((isMLMethodCall(sub) and (not hasExplicitHyperparameters(sub)))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Hyperparameter not explicitly set at line {lineno}", node)
