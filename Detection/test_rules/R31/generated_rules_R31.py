...
def rule_R31(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((((isVisionModelCall(sub) and hasImageContent(sub)) and (not hasImagePreprocessing(sub))) and (not hasExplicitDetailLevel(sub)))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Unbounded vision payloads at line {lineno}", node)
