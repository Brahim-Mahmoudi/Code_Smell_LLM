...
def rule_R10(ast_node):
    ...
    if (( usage_nodes := [ n for n in ast.walk(ast_node) if (isPytorchTensorUsage(n) or isModelCreation(n)) ]) and usage_nodes and not any(isMemoryFreeCall(n2) for n2 in ast.walk(ast_node))):
        report("Memory clearing API not used. Consider calling .detach() for PyTorch or tf.keras.backend.clear_session() for TensorFlow at line")
