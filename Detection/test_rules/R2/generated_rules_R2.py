...
def rule_R2(ast_node):
    ...
    for sub in ast.walk(ast_node):
        if ((((isRandomCall(sub) and (not global_seed_set(ast_node, get_random_lib(sub)))))) or (((isSklearnRandomAlgo(sub) and (not hasRandomState(sub)))))):
            line = getattr(sub, 'lineno', '?')
            if line != '?':
                problems[line] = sub
    for line, node in problems.items():
        report_with_line("Random Seed Not Set at line {lineno}", node)
